<template>
  <div class="content">
    <div class="container">
      ...
      <match-info :name="name" :shard="shard" :id="id" v-if="loaded" v-for="match in matchList" :key="match.id"></match-info>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      requestData () {
        ...
        this.resetState()
        this.loading = true
        axios.get(`http://localhost:8081/api/matches/eu/${this.package}`)
          .then(response => {
      ...
      setURL () {
        history.pushState({ info: `flue ${this.package}` }, this.package, `/test/${this.package}`)
      },
      toggleSettings () {
