<template>
  <div class="content">
    <div class="container">
      ...
      <player-info :name="name" :shard="shard" :id="id" v-if="loaded"></player-info>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      requestData () {
        ...
        this.resetState()
        this.loading = true
        axios.get(`:8081/api/player/eu/${this.package}`)
          .then(response => {
            console.log(response)
            this.rawData = response.data.data
            this.id = response.data.data[0].id
            this.name = response.data.data[0].attributes.name
            this.shard = response.data.data[0].attributes.shardId
            this.stats = response.data.data[0].attributes.stats
            this.setURL()
            this.loaded = true
            this.loading = false
          })
